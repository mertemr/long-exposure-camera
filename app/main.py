...
parser.add_argument(
    "-c", "--camera-id", type=int, help="Camera ID (default: 0)", default=0
)
parser.add_argument(
    "--opacity",
    type=float,
    help="Opacity to blend images (default: 0.3125)",
    default=0.3125,
)
parser.add_argument(
    "--save",
    action="store_true",
    help="Save captured frames to images/ folder",
    default=False,
)
parser.add_argument(
    "-s",
    "--seconds",
    type=int,
    help="Seconds to capturing images (default: 10)",
    default=10,
)
parser.add_argument(
    "--size", help="Image size (default: 640x480)", default="640x480", type=str
)
parser.add_argument(
    "--input-dir",
    action="store_true",
    help="Input directory instead of camera",
    default=False,
)
parser.add_argument(
    "-o",
...
def capture_images() -> numpy.ndarray:
    ...
    captured_frames = numpy.empty(
        (MAX_PHOTO_COUNT, sizeX, sizeY, channels), dtype=numpy.uint8
    )
...
def read_images_from_dir() -> numpy.ndarray:
    ...
    images = [
        i for i in Path(f"images/{directories[choice-1]}").iterdir() if i.is_file()
    ]
    images.sort()
    ...
    captured_frames = numpy.empty(
        (len(images), sizeX, sizeY, channels), dtype=numpy.uint8
    )
